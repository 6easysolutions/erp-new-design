...
const Department = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(URLS.AddHrmsDepartments, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `${URLS.EditHrmsDepartments}${editingId}`,
        editFormData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
                        ...
                        <tr>
                          <td colSpan="5" className="text-center py-5">
                            <div className="text-muted">
                              ...
                              {!searchTerm && departmentsData.length === 0 && (
                                <small className="text-muted">
                                  Click "Submit" to create your first department
                                </small>
                              )}
                            </div>
                          </td>
