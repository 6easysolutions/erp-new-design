...
const Size = () => {
    ...
    const fetchSizes = async () => {
        ...
        try {
            const response = await fetch(URLS.GetSize, {
                method: 'POST',
                headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!sizeName.trim()) { toast.error('Please enter a size name!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(URLS.AddSize, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const handleSaveEdit = async (id) => {
        if (!editValue.trim()) { toast.error('Size name cannot be empty!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.EditSize}${id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const handleDelete = async (id, name) => {
        ...
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.DeleteSize}${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const filteredData  = sizesData.filter(s =>
        ...
        return (
            <div className="pms-m-page-loader">
                <div className="text-center">
                    ...
                    <p className="pms-m-loader-text">Loading sizes...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="pms-m-root">
                <div className="pms-m-main-card">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h1 className="pms-m-title">Size List</h1>
                        ...
                        <div className="col-lg-4">
                            <div className="pms-m-section-card">
                                ...
                                <form onSubmit={handleSubmit}>
                                    <span className="pms-m-field-label">Size Name</span>
                                    ...
                                    <table className="pms-m-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style={{ width: '60%' }}>Size Name</th>
                                                <th style={{ width: '30%', textAlign: 'right' }}>Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {paginatedData.length > 0 ? (
                                                paginatedData.map((size, index) => (
                                                    <tr key={size.id}>
                                                        <td>
                                                        ...
                                                        <td>
                                                            {editingId === size.id ? (
                                                                <input
                                                                    ...
                                                                    disabled={loading}
                                                                    onKeyDown={(e) => {
                                                                        if (e.key === 'Enter')  handleSaveEdit(size.id);
                                                                ...
                                                                {editingId === size.id ? (
                                                                    <>
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleSaveEdit(size.id)}
                                                                            disabled={loading}
                                                                            title="Save"
                                                                        >
                                                                            {loading
                                                                    ...
                                                                    <>
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleEdit(size.id, size.name)}
                                                                            disabled={loading}
                                                                            title="Edit"
                                                                        >
                                                    ...
                                                    <td colSpan="3">
                                                        <div className="pms-m-empty">
                                                            ...
                                                            <span className="pms-m-empty-text">
                                                                {searchTerm ? 'No sizes found matching your search' : 'No sizes to display'}
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
