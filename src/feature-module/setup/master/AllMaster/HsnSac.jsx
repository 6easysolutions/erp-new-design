...
const HsnSac = () => {
    ...
    useEffect(() => { fetchHsnSac(); }, []);

    const fetchHsnSac = async () => {
        setPageLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(URLS.GetHsnSac, {
                method: 'POST',
                headers: {
            ...
            if (response.ok && data.success) {
                setHsnData(data.hsnsac || []); // ← data.hsnsac preserved
            } else {
                toast.error(data.message || 'Failed to fetch HSN/SAC codes');
            }
        } catch {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!hsnName.trim()) { toast.error('Please enter an HSN/SAC code!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(URLS.AddHsnSac, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ name: hsnName }),
            });
            const data = await response.json();
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to add HSN/SAC code');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleSaveEdit = async (id) => {
        if (!editValue.trim()) { toast.error('HSN/SAC code cannot be empty!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.EditHsnSac}${id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to update HSN/SAC code');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleDelete = async (id, name) => {
        ...
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.DeleteHsnSac}${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to delete HSN/SAC code');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const filteredData  = hsnData.filter(h =>
        ...
        return (
            <div className="pms-m-page-loader">
                <div className="text-center">
                    ...
                    <p className="pms-m-loader-text">Loading HSN/SAC codes...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="pms-m-root">
                <div className="pms-m-main-card">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h1 className="pms-m-title">HSN/SAC List</h1>
                        ...
                        <div className="col-lg-4">
                            <div className="pms-m-section-card">
                                ...
                                <form onSubmit={handleSubmit}>
                                    <span className="pms-m-field-label">HSN/SAC Code</span>
                                    <div className="pms-m-field-group">
                                        <input
                                            className="pms-m-field-input"
                                            type="text"
                                            value={hsnName}
                                            onChange={(e) => setHsnName(e.target.value)}
                                    ...
                                    <table className="pms-m-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style={{ width: '60%' }}>HSN/SAC Code</th>
                                                <th style={{ width: '30%', textAlign: 'right' }}>Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {paginatedData.length > 0 ? (
                                                paginatedData.map((hsn, index) => (
                                                    <tr key={hsn.id}>
                                                        <td>
                                                        ...
                                                        <td>
                                                            {editingId === hsn.id ? (
                                                                <input
                                                                    ...
                                                                    disabled={loading}
                                                                    onKeyDown={(e) => {
                                                                        if (e.key === 'Enter')  handleSaveEdit(hsn.id);
                                                                ...
                                                                {editingId === hsn.id ? (
                                                                    ...
                                                                    <>
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleEdit(hsn.id, hsn.name)}
                                                                            disabled={loading}
                                                                            title="Edit"
                                                                        >
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleDelete(hsn.id, hsn.name)}
                                                                            disabled={loading}
                                                                            title="Delete"
                                                                        >
                                                    ...
                                                    <td colSpan="3">
                                                        <div className="pms-m-empty">
                                                            ...
                                                            <span className="pms-m-empty-text">
                                                                {searchTerm ? 'No HSN/SAC codes found matching your search' : 'No HSN/SAC codes to display'}
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
