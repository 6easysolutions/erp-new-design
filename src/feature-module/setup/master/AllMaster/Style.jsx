...
const Style = () => {
    ...
    const fetchStyles = async () => {
        ...
        try {
            const response = await fetch(URLS.GetStyle, {
                method: 'POST',
                headers: {
            ...
            if (response.ok && data.success) {
                setStylesData(data.style || []); // ← data.style preserved
            } else {
                toast.error(data.message || 'Failed to fetch styles');
            }
        } catch {
    ...
    const handleSubmit = async (e) => {
        ...
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(URLS.AddStyle, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to add style');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleSaveEdit = async (id) => {
        if (!editValue.trim()) { toast.error('Style name cannot be empty!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.EditStyle}${id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to update style');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleDelete = async (id, name) => {
        ...
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.DeleteStyle}${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const filteredData  = stylesData.filter(s =>
        ...
        return (
            <div className="pms-m-page-loader">
                <div className="text-center">
                    ...
                    <p className="pms-m-loader-text">Loading styles...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="pms-m-root">
                <div className="pms-m-main-card">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h1 className="pms-m-title">Style List</h1>
                        ...
                        <div className="col-lg-4">
                            <div className="pms-m-section-card">
                                ...
                                <form onSubmit={handleSubmit}>
                                    <span className="pms-m-field-label">Style Name</span>
                                    ...
                                    <table className="pms-m-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style={{ width: '60%' }}>Style Name</th>
                                                <th style={{ width: '30%', textAlign: 'right' }}>Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {paginatedData.length > 0 ? (
                                                paginatedData.map((style, index) => (
                                                    <tr key={style.id}>
                                                        <td>
                                                        ...
                                                        <td>
                                                            {editingId === style.id ? (
                                                                <input
                                                                    ...
                                                                    disabled={loading}
                                                                    onKeyDown={(e) => {
                                                                        if (e.key === 'Enter')  handleSaveEdit(style.id);
                                                    ...
                                                    <td colSpan="3">
                                                        <div className="pms-m-empty">
                                                            ...
                                                            <span className="pms-m-empty-text">
                                                                {searchTerm ? 'No styles found matching your search' : 'No styles to display'}
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
