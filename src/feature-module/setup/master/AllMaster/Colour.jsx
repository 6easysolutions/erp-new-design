...
const Colour = () => {
    ...
    const fetchColours = async () => {
        ...
        try {
            const response = await fetch(URLS.GetColour, {
                method: 'POST',
                headers: {
            ...
            if (response.ok && data.success) {
                setColoursData(data.colors || []);
            } else {
                toast.error(data.message || 'Failed to fetch colours');
            }
        } catch {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!colourName.trim()) { toast.error('Please enter a colour name!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(URLS.AddColour, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ name: colourName }),
            });
            const data = await response.json();
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to add colour');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleSaveEdit = async (id) => {
        if (!editValue.trim()) { toast.error('Colour name cannot be empty!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.EditColour}${id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to update colour');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleDelete = async (id, name) => {
        ...
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.DeleteColour}${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to delete colour');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const filteredData  = coloursData.filter(c =>
        ...
        return (
            <div className="pms-m-page-loader">
                <div className="text-center">
                    ...
                    <p className="pms-m-loader-text">Loading colours...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="pms-m-root">
                <div className="pms-m-main-card">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h1 className="pms-m-title">Colour List</h1>
                        ...
                        <div className="col-lg-4">
                            <div className="pms-m-section-card">
                                ...
                                <form onSubmit={handleSubmit}>
                                    <span className="pms-m-field-label">Colour Name</span>
                                    ...
                                    <table className="pms-m-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style={{ width: '60%' }}>Colour Name</th>
                                                <th style={{ width: '30%', textAlign: 'right' }}>Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {paginatedData.length > 0 ? (
                                                paginatedData.map((colour, index) => (
                                                    <tr key={colour.id}>
                                                        <td>
                                                        ...
                                                        <td>
                                                            {editingId === colour.id ? (
                                                                <input
                                                                    ...
                                                                    disabled={loading}
                                                                    onKeyDown={(e) => {
                                                                        if (e.key === 'Enter')  handleSaveEdit(colour.id);
                                                                ...
                                                                {editingId === colour.id ? (
                                                                    <>
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleSaveEdit(colour.id)}
                                                                            disabled={loading}
                                                                            title="Save"
                                                                        >
                                                                            {loading
                                                                    ...
                                                                    <>
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleEdit(colour.id, colour.name)}
                                                                            disabled={loading}
                                                                            title="Edit"
                                                                        >
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleDelete(colour.id, colour.name)}
                                                                            disabled={loading}
                                                                            title="Delete"
                                                                        >
                                                    ...
                                                    <td colSpan="3">
                                                        <div className="pms-m-empty">
                                                            ...
                                                            <span className="pms-m-empty-text">
                                                                {searchTerm ? 'No colours found matching your search' : 'No colours to display'}
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
