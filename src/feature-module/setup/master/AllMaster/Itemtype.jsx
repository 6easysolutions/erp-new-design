...
const ItemType = () => {
    ...
    useEffect(() => { fetchItemTypes(); }, []);

    const fetchItemTypes = async () => {
        setPageLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(URLS.GetItemType, {
                method: 'POST',
                headers: {
            ...
            if (response.ok && data.success) {
                setItemTypesData(data.itemtype || []); // ← data.itemtype preserved
            } else {
                toast.error(data.message || 'Failed to fetch item types');
            }
        } catch {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!itemType.trim()) { toast.error('Please enter an item type!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(URLS.AddItemType, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to add item type');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleEdit       = (id, type) => { setEditingId(id); setEditValue(type); };
    const handleCancelEdit = ()          => { setEditingId(null); setEditValue(''); };

    const handleSaveEdit = async (id) => {
        if (!editValue.trim()) { toast.error('Item type cannot be empty!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.EditItemType}${id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to update item type');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleDelete = async (id, type) => {
        ...
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.DeleteItemType}${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to delete item type');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const filteredData  = itemTypesData.filter(i =>
        ...
        return (
            <div className="pms-m-page-loader">
                <div className="text-center">
                    ...
                    <p className="pms-m-loader-text">Loading item types...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="pms-m-root">
                <div className="pms-m-main-card">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h1 className="pms-m-title">Item Type List</h1>
                        ...
                        <div className="col-lg-4">
                            <div className="pms-m-section-card">
                                ...
                                <form onSubmit={handleSubmit}>
                                    <span className="pms-m-field-label">Item Type</span>
                                                    ...
                                                    <td colSpan="3">
                                                        <div className="pms-m-empty">
                                                            ...
                                                            <span className="pms-m-empty-text">
                                                                {searchTerm ? 'No item types found matching your search' : 'No item types to display'}
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
