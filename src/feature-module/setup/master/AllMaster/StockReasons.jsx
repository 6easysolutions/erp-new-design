...
const StockReasons = () => {
    ...
    const fetchReasons = async () => {
        ...
        try {
            const response = await fetch(URLS.GetStockReason, {
                method: 'POST',
                headers: {
            ...
            if (response.ok && data.success) {
                setReasonsData(data.data || []); // ← data.data preserved
            } else {
                toast.error(data.message || 'Failed to fetch stock reasons');
            }
        } catch {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!reasonName.trim()) { toast.error('Please enter a reason name!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(URLS.AddStockReason, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ reasonName }), // ← reasonName field preserved
            });
            const data = await response.json();
            if (response.ok && data.success) {
                toast.success('Stock reason added successfully!');
                setReasonName('');
                fetchReasons();
            } else {
                toast.error(data.message || 'Failed to add stock reason');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleSaveEdit = async (id) => {
        if (!editValue.trim()) { toast.error('Reason name cannot be empty!'); return; }
        setLoading(true);
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.EditStockReason}${id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ reasonName: editValue }), // ← reasonName field preserved
            });
            const data = await response.json();
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to update stock reason');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    const handleDelete = async (id, name) => {
        ...
        const token = localStorage.getItem('authToken');
        try {
            const response = await fetch(`${URLS.DeleteStockReason}${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok && data.success) {
                ...
                toast.error(data.message || 'Failed to delete stock reason');
            }
        } catch {
            toast.error('Network error. Please try again.');
        } finally {
    ...
    // ← reasonName field access preserved (with name fallback)
    const filteredData  = reasonsData.filter(r =>
        (r.reasonName || r.name || '').toLowerCase().includes(searchTerm.toLowerCase())
    );
        ...
        return (
            <div className="pms-m-page-loader">
                <div className="text-center">
                    ...
                    <p className="pms-m-loader-text">Loading stock reasons...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="pms-m-root">
                <div className="pms-m-main-card">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h1 className="pms-m-title">Stock Reasons</h1>
                        ...
                        <div className="col-lg-4">
                            <div className="pms-m-section-card">
                                ...
                                <form onSubmit={handleSubmit}>
                                    <span className="pms-m-field-label">Reason Name</span>
                                    ...
                                    <table className="pms-m-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style={{ width: '60%' }}>Reason Name</th>
                                                        ...
                                                        <td>
                                                            {editingId === reason.id ? (
                                                                ...
                                                                <span style={{ fontWeight: 500, color: '#1e293b' }}>
                                                                    {reason.reasonName} {/* ← reasonName preserved */}
                                                                </span>
                                                            )}
                                                        </td>
                                                        <td>
                                                                ...
                                                                {editingId === reason.id ? (
                                                                    ...
                                                                    <>
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleEdit(reason.id, reason.reasonName)}
                                                                            disabled={loading}
                                                                            title="Edit"
                                                                        >
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            onClick={() => handleDelete(reason.id, reason.reasonName)}
                                                                            disabled={loading}
                                                                            title="Delete"
                                                                        >
                                                    ...
                                                    <td colSpan="3">
                                                        <div className="pms-m-empty">
                                                            ...
                                                            <span className="pms-m-empty-text">
                                                                {searchTerm ? 'No matching stock reasons found' : 'No stock reasons to display'}
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
